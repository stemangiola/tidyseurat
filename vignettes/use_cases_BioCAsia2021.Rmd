...
```{r}
library(tidyseurat)
seurat_obj = here("dev", "seurat_obj_light.rds") %>% readRDS()

# seurat_obj_light = seurat_obj %>% filter(.cell %in% (seurat_obj %>% sample_frac(0.5) %>%  pull(.cell) %>% c(seurat_obj %>% filter(grepl("Delta", curated_cell_type)) %>% pull(.cell)) %>% unique))
# 
# seurat_obj_light = seurat_obj 
# 
# seurat_obj_light = seurat_obj_light[VariableFeatures(seurat_obj_light),]
# 
# seurat_obj_light[["SCT"]]@scale.data = seurat_obj_light[["SCT"]]@scale.data[c("CD3D", "TRDC", "TRGC1", "TRGC2", "CD8A", "CD8B"),]
# seurat_obj_light[["SCT"]]@data = seurat_obj_light[["SCT"]]@data[c("CD3D", "TRDC", "TRGC1", "TRGC2", "CD8A", "CD8B"),]
# 
# DefaultAssay(seurat_obj_light) = "SCT"
# seurat_obj_light[["integrated"]] = NULL
# 
# seurat_obj = seurat_obj_light
...
seurat_obj |>
  ...
  mutate( gate = tidygate::gate_int(UMAP_1, UMAP_2, gate_list = here("dev", "gate_for_biocasia2021.rds") %>% readRDS()) ) |> 
